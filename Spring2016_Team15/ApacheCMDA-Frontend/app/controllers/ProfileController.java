...
//        for (User entry : followers)
  //      {
    //        if (entry.getId() == myId)
      //          isFollower = true;
       // }
        UserRespository u1 = new UserRespository(followers);
        for (Iterator iter = u1.getIterator(); iter.hasNext();) {
	    User entry = iter.next();
            if (entry.getId() == myId) 
		isFollower = true;
        }
        UserRespository u2 = new UserRespository(followees);
        for (Iterator iter = u2.getIterator(); iter.hasNext();) {
            User entry = iter.next();
            if (entry.getId() == myId)
                isFollowee = true;
        }
