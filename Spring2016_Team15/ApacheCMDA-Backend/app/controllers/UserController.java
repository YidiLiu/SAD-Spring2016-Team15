...
import util.Common;
import util.Context;
import util.InvalidState;
import util.ValidState;
import util.State;
...
public class UserController extends Controller {
	...
	public Result userRegister() {
        ...
        */
        ...
        //Refactor the register json parsing with factory pattern
        AbstractJsonParser registerJsonParser = JsonParserFactory.getParser("register");
        RegisterObject registerObject = registerJsonParser.getRegisterObject(json);

		try {
			if (userRepository.findByEmail(registerObject.getEmail()) != null) {
				System.out.println("Email has been used: " + registerObject.getEmail());
				return Common.badRequestWrapper("Email has been used");
			}
			User user = new User(registerObject.getName(),
                                registerObject.getEmail(),
                                MD5Hashing(registerObject.getPassword()));
			user.setAvatar(registerObject.getAvatar());
			userRepository.save(user);
			System.out.println("User saved: " + user.getId());
			return created(new Gson().toJson(user.getId()));
		} catch (PersistenceException pe) {
			pe.printStackTrace();
			System.out.println("User not saved: " + registerObject.getName());
			return Common.badRequestWrapper("User not saved: " + registerObject.getName());
		}
	...
	public Result userLogin() {
        ...
        */
        ...
        /* refactor the login json parsing with factory pattern */
        AbstractJsonParser loginJsonParser = JsonParserFactory.getParser("login");
        LoginObject loginObject = loginJsonParser.getLoginObject(json);

        User user = userRepository.findByEmail(loginObject.getEmail());
		if (user.getPassword().equals(MD5Hashing(loginObject.getPassword()))) {
			System.out.println("User is valid");
	/* changed code */
			ValidState v1 = new ValidState();
			v1.validate(context);
			...
			System.out.println("User is not valid");
			/*Changed code*/
			InvalidState v2 = new InvalidState();
			v2.validate(context);
			System.out.println(context.getState().toString());
			return Common.badRequestWrapper("User is not valid");
		}
